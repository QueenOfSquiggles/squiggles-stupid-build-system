...
void StupidBuild::collect_log_data(Source *source, std::filesystem::path log_file)
{
	...
	{
		log_file_line++;
		if (line.substr(0, 4).compare("src/") != 0 || line.find(":") == string::npos)
		{
			continue;
		}
		// source file
...
bool StupidBuild::has_source_changed(Source source)
{
	...
	return filesystem::last_write_time(source.filepath) > filesystem::last_write_time(this->get_obj_path(source));
}
...
BuildResponse StupidBuild::build_target(string target)
{
	...
	for (Source s : this->source_files)
	{
		...
		if (!this->incremental || has_source_changed(s))
		{
			...
			snprintf(cmd, CMD_BUFFER_SIZE, "g++ -c %s %s -o %s &> %s",
					 args.c_str(),
					 rel_path.generic_string().c_str(),
					 get_obj_path(s).c_str(),
					 (log_path).c_str());
			system(cmd);
		}
		collect_log_data(&s, log_path);
